    ...
    if pos is not None:
        ...
        if len(x) > 0 and len(y) > 0:
            ...
            if len(plates) > 0:
                ...
                cv2.imwrite(f"cars/car_{time_now}.jpg", frame)
...
def increase_resolution(scale_factor, image_path):
    image = cv2.imread(f'plates/{image_path}.jpg', cv2.IMREAD_GRAYSCALE)
    ...
    if height < 100 or width < 200:
        ...
        # print(image_path)
        cv2.imwrite(f"plates/{image_path}_resized.jpg", resized_img)
        file_name = f"{image_path}_resized"
        return file_name
    else:
...
def process_image_with_tesseract(image_path):
    ...
    cv2.imwrite(f"{image_path}_binarized.jpg", img)
    text = pytesseract.image_to_string(img, lang='eng', config='--psm 10 -c tessedit_char_whitelist=ABEKkMmHOoPpCcTYyXx0123456789')
    return text
