    ...
    if pos is not None:
        ...
        if len(x) > 0 and len(y) > 0:
            ...
            if len(plates) > 0:
                ...
                cv2.imwrite(f"17_12/plates/{file_name}.jpg", crop)
                return file_name
            else:
...
def rotate_plate(image_path):
    image = cv2.imread(f'17_12/plates/{image_path}.jpg')
    ...
    cv2.imwrite(f"17_12/plates/{image_path}_rotated.jpg", aligned_image)


def delete_background(image_path):
    image = cv2.imread(f'17_12/plates/{image_path}_rotated.jpg')
    ...
    cv2.imwrite(f"17_12/plates/{image_path}_without_background.jpg", result)


def increase_resolution(img, scale_factor, image_path):

    # Увеличение размера с использованием бикубической интерполяции
    ...
    if height < 100 or width < 200:
        ...
        # print(image_path)
        cv2.imwrite(f"{image_path}_resized.jpg", resized_img)
        return resized_img
    else:
...
def process_image_with_tesseract(image_path):
    ...
    text = pytesseract.image_to_string(denoised_img, lang='eng', config='--psm 10 -c tessedit_char_whitelist=ABEKkMmHOoPpCcTYyXx0123456789')
    return text


def recognize_symbols_with_rotated_image(image_path):
    rotate_plate(image_path)
    text = process_image_with_tesseract(f'17_12/plates/{image_path}_rotated.jpg')
    # print(text)
...
def recognize_symbols_with_image_without_background(image_path):
    ...
    text = process_image_with_tesseract(f'17_12/plates/{image_path}_without_background.jpg')
    # print(text)
    return process_text(text)


def recognize_symbols_with_stock_image(image_path):
    text = process_image_with_tesseract(f'17_12/plates/{image_path}.jpg')
    # print(text)
    return process_text(text)
...
# plate = f'17_12/cars/4.jpg'
# plate_recognition(plate)

for i in range(1, 16):
    time.sleep(1)
    # plate_recognition(f'17_12/cars/{i}.jpg')
    plate_recognition(f'17_12/cars/{i}.jpg')